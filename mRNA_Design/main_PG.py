...
energy_model = {'CG': 3, 'GC': 3, 'AU': 2, 'UA': 2, 'GU': 1, 'UG': 1}
sharpturn = 3
INF = 1e32
lr = 1
n_iters = 100
            ...
            if i > 0 and len(codon_table[aa]) > 1:
                print(i, "%4s" % aa,
                      " | ".join("[%s]:%.3f, grad:%.3f" % (c,v,g) for (c,v,g) in zip(codon_table[aa], para.value(), para.gradient()) \
                         if v > -0.01))
                new_para = project_to_simplex(para.value() + lr * para.gradient())
                para.set_value(new_para)
        dummy.set(0.)
    print("[Final Result]: (obj): %.5f\t(seq): %s"  % (realv, get_solution(graph)))
    # save logs to a json file
    json_object = json.dumps(logs, indent=4)
    with open("PG_len{}_lr{}_n_iters{}.json".format(m, lr, n_iters), "w") as outfile:
