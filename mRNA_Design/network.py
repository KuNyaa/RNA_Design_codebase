       ...
       for i in self.nodes:
          if i % 3 > 0: # (i-1) -- (i) is not a 3rd letter
             ...
             for i_node in self.nodes[i]: # TODO: just nodes[0]
                ...
                curr_para.set_value(np.array([1/num_codons] * num_codons))
                self.parameters.append(curr_para)

                for p, (iminus1_node, nuc) in enumerate(lattice.left_edges[i_node]):
                    self.add_edge(iminus1_node, i_node, nuc, curr_para[p])
...
def read_wheel(filename, old=False):
    ...
    for line in open(filename):
       line = line.strip().split("\t") # Leu	U U AG	C U UCAG
       aa = line[0]
       graph = Lattice(aa)
       last_first = None
       codons = []
       for i, option in enumerate(line[1:]):
